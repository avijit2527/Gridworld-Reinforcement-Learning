...
class RunAgents:
    ...
    def reset(self):
        if(self.training):
            ...
            #np.save("./Index_Initialization/index_initialization_%2.2d"%(self.num_agents),indices)
            indices = np.load("./Index_Initialization/index_initialization_%2.2d.npy"%(self.num_agents))
...
num_agents_array = np.arange(2,20)
for num_agents in num_agents_array:

         
    number_of_runs = 100
    coverage_array_for_alpha_over_multiple_runs = []
    for run in range(number_of_runs):
        ...
        alpha_array = np.linspace(0,1,num=20)
        #num_agents = 11
        for alpha in alpha_array:
            beta_array = [1.5] #np.linspace(-20,20,num=50)
            for beta in beta_array:
                print("Run = %d, Beta: %2.2f, Num Agents: %2.2d, Alpha = %2.2f "%(run, beta, num_agents, alpha))
                agents = np.empty([num_agents],dtype = GridWorld) 
                game = RunAgents(10,10,num_agents,True,beta)   
                for i in range(num_agents):
                    agents[i] = GridWorld(epsilon = 0.2,alpha = alpha)
                game.startTraining(agents)
                coverage = game.train(100)
                coverage_array.append([alpha,coverage])
        ...
        coverage_array_for_alpha_over_multiple_runs.append(coverage_array)        
        ...
        plt.ylabel("Coverage")
        #plt.show()  
        if not os.path.exists("./Figure/%s/constant_initialization/%2.2d"%(now,num_agents)):
            os.makedirs("./Figure/%s/constant_initialization/%2.2d"%(now,num_agents))
        plt.savefig("./Figure/%s/constant_initialization/%2.2d/%2.2d.png"%(now,num_agents,run))
        plt.close()          
            
 
 

    coverage_array_for_alpha_over_multiple_runs = np.array(coverage_array_for_alpha_over_multiple_runs)   
    np.save("coverage_vs_alpha_constant_initialization",coverage_array_for_alpha_over_multiple_runs)  
    ...
    plt.title("Coverage vs Alpha for %d Runs"%(number_of_runs))
    plt.xlabel("Alpha")
    plt.ylabel("Coverage")
    plt.savefig("./Figure/%s/constant_initialization/%2.2d/coverage_vs_alpha.png"%(now,num_agents))
    plt.close() 
