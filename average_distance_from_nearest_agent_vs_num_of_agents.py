...
class RunAgents:
    ...
    def __init__(self, width = 10, height=10, num_agents = 2, training = False,beta = -5):
        ...
        self.visited_states = []
        self.distance_from_reward_states_array = []
    ...
    def distance_of_reward_from_nearaest_state(self):
        retval = []
        for state in self.reward_states.keys():
            all_distance = []
            for agent in range(self.num_agents):
                location = self.find_location(agent)
                distance = np.linalg.norm(np.array(state) - np.array(location))
                all_distance.append(distance)
            retval.append(min(all_distance))
    ...
    def step(self, agent, move):
        ...
        distance_from_nearest_agent = self.distance_of_reward_from_nearaest_state()
        for d in  distance_from_nearest_agent:
            self.distance_from_reward_states_array.append(d)
    ...
    def train(self, iterations):
        if(self.training):
            ...
            self.distance_from_reward_states_array = np.array(self.distance_from_reward_states_array)           
            coverage = self.calculate_coverage(self.k_coverage)   
            return np.mean(self.distance_from_reward_states_array)                
...
number_of_runs = 100
distance_from_reward_states_array_over_multiple_runs = []
for run in range(number_of_runs):
                    

    distance_from_reward_states_array = []            
            
            
     
    print("Run No. %d"%(run))
    num_agents_array = np.arange(2,20)       #Number of agents in the grid
    for num_agents in num_agents_array:
        ...
        for beta in beta_array:
            ...
            distance_from_reward_states = game.train(100)
            distance_from_reward_states_array.append([num_agents,distance_from_reward_states])
            game.saveStates()
    ...
    distance_from_reward_states_array_over_multiple_runs.append(distance_from_reward_states_array)        
    #Plotting coverage vs number of agents
    distance_from_reward_states_array = np.array(distance_from_reward_states_array)
    fig, ax = plt.subplots()
    ax.plot(distance_from_reward_states_array.T[0],distance_from_reward_states_array.T[1])
    ...
    #plt.savefig("./Figure/%s/Average_Distance_from_Reward_States_%2.2d.png"%(now,run))
    plt.close()          
            
    

distance_from_reward_states_array_over_multiple_runs = np.array(distance_from_reward_states_array_over_multiple_runs)   
np.save("Average_Distance_from_Reward_States_vs_numAgents_proximity_instant_reward",distance_from_reward_states_array_over_multiple_runs)  



distance_from_reward_states_array_over_multiple_runs = np.load("./Average_Distance_from_Reward_States_vs_numAgents_proximity_instant_reward.npy")


mean_distance_from_reward_states_array = np.mean(distance_from_reward_states_array_over_multiple_runs, axis = 0)
std_distance_from_reward_states_array = np.std(distance_from_reward_states_array_over_multiple_runs,axis = 0)

#Plotting coverage vs number of agents
fig, ax = plt.subplots()
ax.plot(mean_distance_from_reward_states_array.T[0],mean_distance_from_reward_states_array.T[1], c='r')
ax.fill_between(mean_distance_from_reward_states_array.T[0],mean_distance_from_reward_states_array.T[1] - std_distance_from_reward_states_array.T[1],mean_distance_from_reward_states_array.T[1] + std_distance_from_reward_states_array.T[1],alpha = 0.1)
plt.title("Average_Distance_from_Reward_States vs Num_of_Agents for %d Runs"%(number_of_runs))
plt.xlabel("Num of Agents")
plt.ylabel("Average_Distance_from_Reward_States")
